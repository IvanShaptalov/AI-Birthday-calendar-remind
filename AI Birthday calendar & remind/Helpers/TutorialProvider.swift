...
class TutorialProvider {
    static func getCards() -> [MainEvent]{
        ...
        return [.init(eventDate: c.date(bySetting: .month, value: 1, of: .now)!, eventType: .birthday, title: "✏️ Remove tutorial", id: UUID().uuidString),
                .init(eventDate: c.date(bySetting: .month, value: 3, of: .now)!, eventType: .simpleEvent, title: "✨ Create unique wishes", id: UUID().uuidString),
                .init(eventDate: c.date(bySetting: .month, value: 7, of: .now)!, eventType: .anniversary, title: "🚛 import & export birthdays", id: UUID().uuidString),
